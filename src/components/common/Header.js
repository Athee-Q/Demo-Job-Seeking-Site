...
export default function Header() {
  ...
  const navigationLinks = [
    ...
    { name: "Jobs", href: "/jobs", icon: <MdWork className="text-lg" /> },
          ...
          {status === "authenticated" && (
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="focus:outline-none ring-3 ring--(--color-text) rounded-full "
              >
                <FaUserCircle className="text-5xl text-(--color-text) dark:text-(--color-dark-text)" />
              </button>

              {/* Dropdown Menu */}
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-44 bg-white dark:bg-gray-900 rounded-md shadow-lg py-2 z-50 animate-fade-in">
                  <Link
                    href="/profile"
                    className="block px-4 py-2 text-(--color-text) dark:text-(--color-dark-text) hover:bg-gray-100 dark:hover:bg-gray-700"
                  >
                    Profile
                  </Link>
                  <Link
